...
class NeoCortex:
    ...
    class ActionPredictor:
        ...
        def close(self, learn):
            if self.model_file is not None and learn:
    ...
    class Selector:
        ...
        def step(self, selector_input):
            ...
            if max_val > 0.0:
                ...
                self.selection[np.random.randint(0, len(self.selection))] = 1  # one-hot
    ...
    def close(self):
        self.action_predictor.close(self.learn_mode)
...
class BG:
    def __init__(self, config, learning_mode, train):
        ...
        if learning_mode == "rl":
            self.dqn = DQN(config['BG'], config['in_dim'] + config['n_action'], 2)
        self.successes = 0
    ...
    def step(self, in_data, selection, reward, done):
        ...
        observation = np.append(in_data, selection)
        action = 0
        if self.learning_mode == "rl":
            action_t = torch.from_numpy(np.array([0]).astype(np.float32)).clone()
            if self.init:
                action_t = self.dqn.select_action(observation)
                self.prev_action = action_t
                if self.one_go_per_episode:
                    self.dqn.learn(observation, self.prev_action, observation, reward, False, False)
                self.init = False
            else:
                if not self.one_go_per_episode:
                    self.dqn.learn(self.prev_observation, self.prev_action, observation, reward, done, done)
                    action_t = self.dqn.select_action(observation)
                    self.prev_action = action_t
                else:
                    if np.any(self.prev_observation != observation) and done != 1:
                        self.dqn.learn(self.prev_observation, self.prev_action, observation, reward, False, False)
                if done == 1:
                    if self.one_go_per_episode:
                        self.dqn.learn(self.prev_observation, self.prev_action, observation, reward, True, True)
                    self.init = True
            self.dump(self.train['dump'], reward, observation, self.prev_action.item(), done, self.train['dump_flags'])
...
class CBT1Component(brica1.brica_gym.Component):
    ...
    def __init__(self, learning_mode, train, config):
        ...
        config['NeoCortex']['learn'] = config['neocortex_learn']
...
def main():
    ...
    parser.add_argument('--config', type=str, default='CBT1CA3.json', metavar='N',
                        help='Model configuration (default: CBT1CA3.json')
